...
export default function LineWaves({
  ...
  useEffect(() => {
    ...
    // Safety check in case OGL fails to init
    let renderer;
    try {
      renderer = new Renderer({ 
        alpha: true, 
        premultipliedAlpha: false,
        dpr: Math.min(window.devicePixelRatio || 1, 1.25) // Limit maximum pixel scale to reduce GPU overhead
      });
    } catch(err) {
      console.error('LineWaves WebGL Init Error:', err);
      return;
    }
    ...
    function update(time) {
      ...
      // Hardware saver: do absolutely nothing if scrolled away
